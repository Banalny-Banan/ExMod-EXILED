// -----------------------------------------------------------------------
// <copyright file="CommandLogging.cs" company="ExMod Team">
    ...
    internal class CommandLogging
    {
        ...
        public static void LogCommand(string query, CommandSender sender)
        {
            try
            {
                ...
                {
                    logMessage =
                        $"[{DateTime.Now}] {(player == Server.Host ? $"Server Console ({sender.SenderId})" : $"{player?.Nickname} ({player?.UserId}) {player?.IPAddress}")}" +
